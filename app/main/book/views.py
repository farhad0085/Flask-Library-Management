...
def add():
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        flash(u'The book %s has been added to the library!' % new_book.title, 'success')
...
def delete(book_id):
    ...
    db.session.commit()
    flash(u'Successfully deleted the book, the users can no longer view the book', 'info')
