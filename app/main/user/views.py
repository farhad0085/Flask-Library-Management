...
def edit(user_id):
    if current_user.id == user_id or current_user.can(Permission.UPDATE_OTHERS_INFORMATION):
        ...
        if form.validate_on_submit():
            ...
            db.session.commit()
            flash(u'Profile updated successfully!', "info")
        ...
        return render_template('user_edit.html', form=form, user=the_user, title=u"Edit Profile")
    else:
        abort(403)
    ...
    if current_user.id == user_id or current_user.can(Permission.UPDATE_OTHERS_INFORMATION):
        ...
        if avatar_upload_form.validate_on_submit():
            if 'avatar' in request.files:
                ...
                db.session.add(the_user)
                db.session.commit()
                flash(u'Profile Picture updated Successfully!', 'success')
                return redirect(url_for('user.detail', user_id=user_id))
        ...
        return render_template('avatar_edit.html', user=the_user, avatar_edit_form=avatar_edit_form,
                               avatar_upload_form=avatar_upload_form, title=u"Change profile picture")
    else:
        abort(403)
